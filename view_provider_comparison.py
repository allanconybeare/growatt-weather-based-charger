...
def getConfigPath() -> str:
    ...
    config_path = os.getenv("GROWATT_CONFIG", "/opt/growatt-charger/conf/growatt-charger.ini")
...
def view_provider_comparison(days=14):
    ...
    print(
        f"{'Date':<12} {'Primary':<10} {'Solcast':<10} {'F.Solar':<10} {'Actual':<10} {'Variance'}"
    )
    print(f"{'-'*90}")
    ...
    for row in forecasts:
        ...
        print(
            f"{date:<12} {primary_marker}{primary:<9} {solcast_str:<18} "
            f"{fsolar_str:<18} {actual_str:<10} {variance}%"
        )
    ...
    if solcast_accuracies or fsolar_accuracies:
        ...
        if solcast_accuracies and fsolar_accuracies:
            ...
            if abs(diff) < 5:
                print("  Both providers perform similarly")
                ...
                print("  ✓ Solcast is more accurate")
                print(f"  Solcast avg error: {avg_solcast_error:.1f}%")
                print(f"  Forecast.Solar avg error: {avg_fsolar_error:.1f}%")
                if providers_config.primary_provider == "solcast":
                    print("  Recommendation: Keep Solcast as primary")
                else:
                    print("  Recommendation: Consider switching to Solcast as primary")
            else:
                print("  ✓ Forecast.Solar is more accurate")
                print(f"  Solcast avg error: {avg_solcast_error:.1f}%")
                print(f"  Forecast.Solar avg error: {avg_fsolar_error:.1f}%")
                if providers_config.primary_provider == "forecast.solar":
                    print("  Recommendation: Keep Forecast.Solar as primary")
                else:
                    print("  Recommendation: Consider switching to Forecast.Solar as primary")
    else:
        print("\nInsufficient data for accuracy comparison.")
        print("Need at least one matched forecast-to-actual datapoint.")
