    ...
    try:
        ...
        if len([f for f in all_forecasts.values() if f is not None]) > 1:
            ...
            if len(valid_forecasts) >= 2:
                ...
                if variance_pct < 10:
                    print("   ✓ Providers agree closely")
                elif variance_pct < 25:
                    print("   ⚠ Moderate disagreement between providers")
                else:
                    print("   ✗ Significant disagreement - check configuration")
        ...
        print(
            f"\nPrimary provider ({manager.primary_provider_name}) "
            ""
            "will be used for charging decisions."
        )
