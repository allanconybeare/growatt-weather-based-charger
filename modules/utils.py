...
def get_grid_neutral_time(
    average_load_w: float, generation_forecast: Dict[datetime, float]
) -> Optional[datetime]:
    """
...
def get_grid_neutral_wh(
    grid_neutral_time: datetime, today_off_peak_end: datetime, average_load_w: float
) -> float:
    """
    ...
    duration = datetime.combine(datetime.min, grid_neutral_time.time()) - datetime.combine(
        datetime.min, today_off_peak_end.time()
    )
...
def get_surplus_generation_for_battery(
    generation_forecast: Dict[datetime, float], average_load_w: float, maximum_charge_rate_w: float
) -> float:
    """
