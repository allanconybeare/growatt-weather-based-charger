...
def main():
    ...
    try:
        ...
        if prediction:
            ...
            charge_rate_key = (
                "Charge Rate Set (%)" if "Charge Rate Set (%)" in prediction else "Charge Rate (%)"
            )
            ...
            log_morning_soc(output_dir, today, actual_soc, target_soc, charge_rate_set, variance)
            ...
            if abs(variance) <= 5:
                print("Status: ✓ Excellent (within 5%)")
            elif abs(variance) <= 10:
                print("Status: ✓ Good (within 10%)")
            elif abs(variance) <= 15:
                print("Status: ⚠ Fair (within 15%)")
            else:
            ...
            print("No prediction available for comparison.")
            print("(Predictions are made the night before at 22:00)")
            print(f"{'='*60}\n")
