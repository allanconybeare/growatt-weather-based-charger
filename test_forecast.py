...
def main():
    ...
    try:
        ...
        print(f"   {'-' * 55}")
        print("   Current | Target | Rate | Solar Coverage")
        print("   SOC (%) | SOC(%) | (%)  | (%)")
        print(f"   {'-' * 55}")

        for current_soc in test_soc_levels:
            charge_plan = calculator.calculate_optimal_charge_plan(
                current_soc=current_soc, forecast_wh=forecast_wh
            )

            print(
                f"   {current_soc:>7} | {charge_plan['target_soc']:>6} | "
        ...
        print("   (Assuming current SOC is 50% for this test)")

        charge_plan = calculator.calculate_optimal_charge_plan(
            current_soc=50.0, forecast_wh=forecast_wh
        )

        print("\n   Forecast Summary:")
        print(
            f"   - Tomorrow's generation: {charge_plan['forecast_wh']:.0f}Wh "
