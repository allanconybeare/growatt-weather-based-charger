...
def view_morning_soc_results(days=7):
    ...
    print(
        f"{'Date':<12} {'Target':<8} {'Actual':<8} {'Variance':<10} "
        f"{'Rate':<6} {'Achievement':<12} {'Status'}"
    )
    print(f"{'-'*80}")
    ...
    if count > 0:
        ...
        if avg_achievement >= 95:
            print("  Status: ✓ Excellent - Charging is highly effective")
        elif avg_achievement >= 85:
            print("  Status: ✓ Good - Charging is working well")
        elif avg_achievement >= 75:
            print("  Status: ⚠ Fair - Consider reviewing charge rate")
        else:
            print("  Status: ✗ Poor - Charging significantly underperforming")
